...
namespace GoBike.API.Service.Managers.Member
{
    ...
    public class MemberService : IMemberService
    {
        ...
        public async Task<EditDataRespone> EditData(EditDataRequest editRequest)
        {
            try
            {
                MemberData memberData = await this.memberRepository.GetMemebrDataByID(editRequest.MemberID);
                if (memberData == null)
                {
                    return new EditDataRespone() { ResultCode = -1, ResultMessage = "The member is not exist." };
                }

                memberData.BirthDayDate = string.IsNullOrEmpty(editRequest.BirthDayDate) ? memberData.BirthDayDate : editRequest.BirthDayDate;
                memberData.BodyHeight = string.IsNullOrEmpty(editRequest.BodyHeight) ? memberData.BodyHeight : Convert.ToDecimal(editRequest.BodyHeight);
                memberData.BodyWeight = string.IsNullOrEmpty(editRequest.BodyWeight) ? memberData.BodyWeight : Convert.ToDecimal(editRequest.BodyWeight);
                memberData.Gender = string.IsNullOrEmpty(editRequest.Gender) ? memberData.Gender : editRequest.Gender;
        ...
        public async Task<ForgetPasswordRespone> ForgetPassword(string email)
        {
            try
            {
        ...
        public async Task<GetMemberInfoRespone> GetMemberInfo(string memberID)
        {
            try
            {
        ...
        public async Task<LoginRespone> Login(string email, string password)
        {
            try
            {
        ...
        /// <returns>Tuple</returns>
        public async Task<LoginRespone> Login(string token)
        {
        ...
        /// <param name="password">password</param>
        /// <returns>Tuple</returns>
        public async Task<RegisterRespone> Register(string email, string password)
        {
            try
            {
                if (string.IsNullOrEmpty(email) || string.IsNullOrEmpty(password))
