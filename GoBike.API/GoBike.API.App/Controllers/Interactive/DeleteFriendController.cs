...
namespace GoBike.API.App.Controllers.Interactive
{
    ...
    public class DeleteFriendController : ApiController
    {
        ...
        public async Task<IActionResult> Post(MemberBaseDto memberBase)
        {
            ...
            try
            {
                ResponseResultDto responseResultDto = await this.interactiveService.DeleteFriend(new InteractiveCommandDto() { InitiatorID = memberID, ReceiverID = memberBase.MemberID });
            ...
            {
                this.logger.LogError($"Delete Friend Error >>> InitiatorID:{memberID} ReceiverID:{memberBase.MemberID}\n{ex}");
