...
namespace GoBike.API.App.Controllers.Interactive
{
    ...
    public class AddFriendController : ApiController
    {
        ...
        public AddFriendController(ILogger<AddFriendController> logger, IMemberService memberService)
        {
            this.logger = logger;
        ...
        public async Task<IActionResult> Post(MemberBaseCommandDto memberBaseCommand)
        {
            ...
            try
            {
                ResponseResultDto responseResult = await this.memberService.AddFriend(new MemberInteractiveCommandDto() { InitiatorID = memberID, ReceiverID = memberBaseCommand.MemberID });
