...
namespace GoBike.API.App.Controllers.Interactive
{
    ...
    public class AddFriendController : ApiController
    {
        ...
        public async Task<IActionResult> Post(MemberInfoDto memberInfo)
        {
            ...
            try
            {
                ResponseResultDto responseResultDto = await this.interactiveService.AddFriend(new InteractiveInfoDto() { InitiatorID = memberID, PassiveID = memberInfo.MemberID });
            ...
            {
                this.logger.LogError($"Add Friend Error >>> InitiatorID:{memberID} PassiveID:{memberInfo.MemberID}\n{ex}");
