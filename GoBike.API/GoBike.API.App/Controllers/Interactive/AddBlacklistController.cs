...
namespace GoBike.API.App.Controllers.Interactive
{
    ...
    public class AddBlacklistController : ApiController
    {
        ...
        public AddBlacklistController(ILogger<AddBlacklistController> logger, IMemberService memberService)
        {
            this.logger = logger;
        ...
        public async Task<IActionResult> Post(MemberBaseCommandDto memberBaseCommand)
        {
            ...
            try
            {
                ResponseResultDto responseResult = await this.memberService.AddBlacklist(new MemberInteractiveCommandDto() { InitiatorID = memberID, ReceiverID = memberBaseCommand.MemberID });
